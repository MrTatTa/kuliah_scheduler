...
class DatabaseService {
  ...
  static List<Event> getEventsByDate(DateTime date) {
    return _event.values.where((e) {
      return e.tanggal.year == date.year &&
  ...
  static Map<StatusPresensi, int> getRekapPresensi(String kelasId) {
    ...
    return {
      StatusPresensi.hadir: list.where((p) => p.status == StatusPresensi.hadir).length,
      StatusPresensi.izin: list.where((p) => p.status == StatusPresensi.izin).length,
      StatusPresensi.sakit: list.where((p) => p.status == StatusPresensi.sakit).length,
      StatusPresensi.alpa: list.where((p) => p.status == StatusPresensi.alpa).length,
    };
  }
  ...
  static Future<void> deleteJurnal(String id) async =>
      await _jurnal.delete(id);

  // ── Quest ─────────────────────────────────────────────────────────────────

  static Box<Quest> get _quest => Hive.box<Quest>(_questBox);

  static List<Quest> getAllQuests() => _quest.values.toList()
    ..sort((a, b) => a.deadline.compareTo(b.deadline));

  static List<Quest> getActiveQuests() => _quest.values
      .where((q) => q.status != StatusQuest.selesai)
      .toList()
        ..sort((a, b) => a.deadline.compareTo(b.deadline));

  static List<Quest> getCompletedQuests() => _quest.values
      .where((q) => q.status == StatusQuest.selesai)
      .toList();
